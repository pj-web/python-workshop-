...
# проверяем чтобы слова не заканчивались на 'ь' 'ы' 'й'
    ...
    if user_input in cities_set:  # проверяем наличие введенного названия во множестве
        cities_set.remove(user_input)  # удаляем введенное название из множества
        print(f'Мой вариант на {user_input[-1].upper()}')

# основной цикл игры
for city in list(cities_set):
    # выбираем название города для хода компьютера
    if city in cities_set and city[0] == user_input[-1] and city[-1] != 'ь' and city[-1] != 'ы' and city[-1] != 'й':
        cities_set.remove(city)  # удаляем выбранное название из множества городов
        ...
        if (user_input in cities_set and user_input[0] == city[-1]
